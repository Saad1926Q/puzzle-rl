...
`saad1926q/8-puzzle` (the full split by default). Each turn sends DeepSeek only the
current board and requests one strict `submit_move` tool call. Thinking mode is enabled
by default with a 4096-token budget; use `--no-thinking` to force the tool call without
reasoning.

Run a real DeepSeek V4 Flash-compatible endpoint using the key in `.env`:
