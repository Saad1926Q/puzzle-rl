...
from rl_training.async_worker import PuzzleAsyncRolloutWorker, build_puzzle_worker_kwargs
from rl_training.dataset import load_puzzle_dataset
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dataset", default=defaults.get("dataset", "data/eval_puzzles_31.jsonl"))
    ...
    parser.add_argument("--output-dir", type=Path, default=Path(defaults.get("output_dir", "outputs/rl")))
    parser.add_argument("--vllm-server-url", default=defaults.get("vllm_server_url", "http://localhost:8000"))
    parser.add_argument("--max-steps", type=int, default=defaults.get("max_steps", 100))
    parser.add_argument("--save-steps", type=int, default=defaults.get("save_steps", 20))
    ...
    parser.add_argument("--num-generations", type=int, default=defaults.get("num_generations", 8))
    parser.add_argument("--max-completion-length", type=int, default=defaults.get("max_completion_length", 512))
    parser.add_argument("--max-turns", type=int, default=defaults.get("max_turns", 45))
    parser.add_argument("--history-turns", type=int, default=defaults.get("history_turns", 4))
    parser.add_argument("--max-inflight-tasks", type=int, default=defaults.get("max_inflight_tasks", 16))
    parser.add_argument("--max-staleness", type=int, default=defaults.get("max_staleness", 1))
    parser.add_argument("--weight-sync-steps", type=int, default=defaults.get("weight_sync_steps", 1))
    parser.add_argument("--learning-rate", type=float, default=defaults.get("learning_rate", 1e-6))
...
def main() -> None:
    """Run bounded-history asynchronous GRPO."""
    ...
    worker_kwargs = build_puzzle_worker_kwargs(
    ...
    worker = PuzzleAsyncRolloutWorker(**worker_kwargs)
